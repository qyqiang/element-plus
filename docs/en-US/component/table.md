...
:::demo Use `ghost-table`, `edit-table`, and column `edit-cell` slots together with `show-add-column-trigger` and `show-add-row-trigger`. The bottom ghost row is shown by default and can be hidden with `show-ghost-row`. Newly inserted structure and the ghost row stay editable through the same cell rendering flow. In multi-column ghost rows, the leading cell stays editable while the trailing cell renders the built-in add action. Columns marked with `required` automatically drive empty editable inputs into the built-in `ElInput` error state, and the table instance exposes `validateRequiredColumns()` so submit handlers can block incomplete rows. You can also listen to `add-ghost-row` to turn the draft row into real table data.
...
Hover near a header or row divider to show an add button, then insert a new column or row after the current position.

:::demo Use `show-add-column-trigger` and `show-add-row-trigger` together with data-driven columns and rows. When the trigger is clicked, the `add-column` and `add-row` events return the current position and insertion index so you can update your own arrays.

table/add-column-trigger

:::

## Control insertion before the first column

Use `allow-insert-before-first-column` on the first `el-table-column` to control whether hovering the left half of its header can insert before it. When this prop is `false`, no add-column trigger is shown on that left-half area.

:::demo Toggle the switch, then hover the left half of the first header cell to compare whether the add-column trigger appears.

table/add-column-first-column-control
...
By default, clicking a sortable column cycles through default, descending, ascending, and back to default.
...
:::demo Set `diagonal-header` on the target `el-table-column` with `from` and `to` fields. That column header will render the diagonal layout automatically.
...
| have-table-text                    | whether to display the default table text footer                                                                                                                                                                                                                           | ^[boolean]                                                                                                                                                           | false                                                                                                                   |
| total                              | total item count shown in the default table text footer                                                                                                                                                                                                                    | ^[number]                                                                                                                                                            | 0                                                                                                                       |
| update-time                        | update time shown in the default table text footer                                                                                                                                                                                                                         | ^[string]                                                                                                                                                            | ''                                                                                                                      |
...
| show-ghost-row                     | whether to show the bottom ghost row when ghost table editing is enabled                                                                                                                                                                                                   | ^[boolean]                                                                                                                                                           | true                                                                                                                    |
...
| show-add-column-trigger            | whether to show an add-column trigger when hovering a header divider, works best when `border` is `true`                                                                                                                                                                   | ^[boolean]                                                                                                                                                           | false                                                                                                                   |
...
| show-add-row-trigger               | whether to show an add-row trigger when hovering a row divider, works best when `border` is `true`                                                                                                                                                                         | ^[boolean]                                                                                                                                                           | false                                                                                                                   |
...
| add-column         | triggers when the add-column button on a header divider is clicked                                                                           | ^[Function]`(payload: { column: TableColumnCtx<T>, columnIndex: number, insertIndex: number, event: MouseEvent }) => void` |
| add-row            | triggers when the add-row button on a row divider is clicked                                                                                 | ^[Function]`(payload: { row: any, rowIndex: number, insertIndex: number, event: MouseEvent }) => void`                     |
| add-ghost-row      | triggers when the built-in add action in the ghost row is clicked                                                                            | ^[Function]`(payload: { row: any, rowIndex: number, rowKey: string, event: MouseEvent }) => void`                          |
...
| sort-orders                      | the order of the sorting strategies used when sorting the data, works when `sortable` is `true`. Accepts an array, as the user clicks on the header, the column is sorted in order of the elements in the array    | ^[object]`('ascending' \| 'descending' \| null)[]`                                                                                                                          | ['descending', 'ascending', null] |
| resizable                        | whether column width can be resized, works when `border` of `el-table` is `true` and `width` is not set                                                                                                            | ^[boolean]                                                                                                                                                                  | true                              |
...
| required                         | whether the column should receive the `required-column` class and participate in built-in required-value validation for editable inputs                                                                            | ^[boolean]                                                                                                                                                                  | false                             |
| allow-insert-before-first-column | whether the first column can show the add-column trigger on the left half of its header to insert before itself                                                                                                    | ^[boolean]                                                                                                                                                                  | true                              |
...
| edit-cell            | Custom editable content for ghost table cells. The slot scope includes `column`, so ghost-row inputs can reuse the header `label` as placeholder text. When the slot returns `ElInput` and the column is `required`, empty values automatically receive the built-in input error state. | ^[object]`{ row: any, column: TableColumnCtx<T>, $index: number }` |
