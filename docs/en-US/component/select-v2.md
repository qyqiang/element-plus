...
Select-V2 supports the same status background behavior as Input through `input-type="error" | "warning" | "info"`. `error` and `warning` show the tinted background while empty, and the background returns to white once the component is focused or has a value. When the select is empty and `input-type` is `error`, focusing or clicking the component shows an outer error tooltip with `Required`. The tooltip closes when the component loses focus. If the select is inside `el-form-item` with a validation error, that form error message takes priority. This status mode does not add a built-in info icon or `info-tip`.

:::demo

select-v2/status-input
...
Add a `tip` field to an option to provide supplementary tooltip content. Option tooltips stay disabled until the pointer enters an option; the component then measures only that option and enables its tooltip when the label overflows or `tip` is present. The tooltip remains available when option content is customized through the `default` slot. If the option label overflows, the tooltip displays the full label and `tip` on separate lines; otherwise it displays only `tip`. Disabled options still display the tooltip; `disabled` only prevents selection. Set `show-option-tooltip` to `false` to disable both overflow and `tip` tooltips for Select V2.
...
| show-option-tooltip                 | whether to show tooltips for overflowing option labels and option `tip` content                                                             | ^[boolean]                                                                                                                                                                  | true                                           |
| input-type                          | visual status style of select, supports `error`, `warning`, and `info`                                                                      | ^[enum]`'error' \| 'warning' \| 'info'`                                                                                                                                     | —                                              |
...
| before-change                       | hook before the selected value changes. If `false` is returned or a `Promise` is returned and then is rejected, selection will be prevented | ^[Function]`(value: any, oldValue: any) => Promise<boolean> \| boolean`                                                                                                     | —                                              |
...
| fallback-placements ^(2.5.6)        | list of possible positions for dropdown popper.js                                                                                           | ^[array]`Placement[]`                                                                                                                                                       | ['bottom-start', 'top-start', 'right', 'left'] |
