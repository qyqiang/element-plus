...
export default defineComponent({
  ...
  render() {
    ...
    const rows = data.reduce((acc: VNode[], row) => {
    ...
    const bodyRows = shouldRenderGhostRow
      ? (() => {
          const ghostRow =
            this.context.ghostRowData?.value ?? this.context.ghostRowData
          return rows.concat(wrappedRowRender(ghostRow, rows.length) as VNode[])
        })()
      : rows
    ...
    return h('tbody', { tabIndex: -1 }, [bodyRows])
  },
})
